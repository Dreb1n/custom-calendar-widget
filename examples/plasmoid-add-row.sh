#!/usr/bin/env bash
# Add a new row or shape to a targeted widget
# Usage:
...
if [ -z "$ROW_JSON" ]; then
    echo "Usage: plasmoid-add-row <targetWidgetId> '<jsonRowObject>'"
    exit 1
fi
    ...
    for (var j = 0; j < w.length; j++) {
        if (w[j].type === 'org.kde.customcalendarwidget') {
            ...
            if (!targetWid || wid === targetWid) {
                if (w[j].rootItem && w[j].rootItem.addRow) {
                    result = w[j].rootItem.addRow(rowJson, targetWid);
                }
            }
        }
    }
